...
static inline void shutter_init(void)
{
...
static inline void shutter_control(uint8_t enable)
{
	m_shutter_active = enable;
	if (!enable) m_shutter_seq = 0;
}

static inline void shutter_set(uint32_t shutter_interval, uint32_t exposure)
{
	global_data.param[PARAM_CAM_INTERVAL] = shutter_interval;
	global_data.param[PARAM_CAM_EXP] = exposure;
}

static inline uint32_t shutter_get_seq(void)
{
	return m_shutter_seq;
}

static inline bool shutter_loop(void)
{	
	bool ret = false;
	if (m_shutter_active)
	{
