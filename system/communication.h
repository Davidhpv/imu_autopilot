...
static void execute_action(uint8_t action)
{
	...
	case MAV_ACTION_LAUNCH:
		if (global_data.mode > (uint8_t)MAV_MODE_LOCKED)
		{
			global_data.status = (uint8_t)MAV_STATE_ACTIVE;
		}
		break;
	...
	case MAV_ACTION_MOTORS_STOP:
		global_data.status = (uint8_t)MAV_STATE_STANDBY;
		break;
	case MAV_ACTION_EMCY_KILL:
		global_data.status = (uint8_t)MAV_STATE_EMERGENCY;
		break;
...
static bool set_mode(uint8_t target, uint8_t mode)
{
	bool ret = true;
	if (target != (uint8_t) global_data.param[PARAM_SYSTEM_ID])
	{
		ret = false;
	}
	else
	{
		debug_message_buffer("MODE update received");
		switch (mode)
		{
		case (uint8_t) MAV_MODE_LOCKED:
			global_data.mode = (uint8_t) MAV_MODE_LOCKED;
			ret = true;
			break;
		case (uint8_t) MAV_MODE_MANUAL:
			global_data.mode = (uint8_t) MAV_MODE_MANUAL;
			ret = true;
			break;
		case (uint8_t) MAV_MODE_GUIDED:
			global_data.mode = (uint8_t) MAV_MODE_GUIDED;
			ret = true;
			break;
		case (uint8_t) MAV_MODE_AUTO:
			global_data.mode = (uint8_t) MAV_MODE_AUTO;
			ret = true;
			break;
		case (uint8_t) MAV_MODE_TEST1:
			global_data.mode = (uint8_t) MAV_MODE_TEST1;
			ret = true;
			break;
		case (uint8_t) MAV_MODE_TEST2:
			global_data.mode = (uint8_t) MAV_MODE_TEST2;
			ret = true;
			break;
		case (uint8_t) MAV_MODE_TEST3:
			global_data.mode = (uint8_t) MAV_MODE_TEST3;
			ret = true;
			break;
		default:
			// Should never be reached, ignore unknown commands
			debug_message_buffer("WARNING: Rejected to set unknown mode");
			ret = false;
			break;
		}
	}
		...
		mavlink_message_t* msg)
{
	...
	case MAVLINK_MSG_ID_SET_MODE:
	{
		...
		{
			set_mode(mode.target, mode.mode);
	...
	case MAVLINK_MSG_ID_IMAGE_TRIGGER_CONTROL:
	{
		...
		debug_message_buffer_sprintf("image_trigger_control_get_enable. %i",
				enable);
	}
	break;
		...
		if (sp.target_system == global_data.param[PARAM_SYSTEM_ID])
		{
			if (global_data.param[PARAM_POSITIONSETPOINT_ACCEPT] == 1)
			{
						...
						<= global_data.position_setpoint_max.z)
				{
					...
					if (global_data.status == MAV_STATE_EMERGENCY
							|| global_data.status == MAV_STATE_CRITICAL)
					{
...
static inline void communication_init(void)
{
